...
const FeedPage = () => {
  ...
  const NGROK_BASE = "https://aurorally-triapsidal-hattie.ngrok-free.dev";
  const CONTROL_API_URL = `${NGROK_BASE}/control`;
  ...
  useEffect(() => {
    ...
    // Don't connect if camera is not enabled
    if (!cameraEnabled) return;

    const liveSocket = new WebSocket("wss://aurorally-triapsidal-hattie.ngrok-free.dev/share") as any;
    ...
    liveSocket.onmessage = async (event: any) => {
      ...
      img.onload = async () => {
        ...
        const drawPredictions = (preds: any[]) => {
          preds.forEach((pred: any) => {
            const { x, y, width, height, confidence, class: className } = pred;
            
            // Calculate top-left corner (x,y are center coordinates)
            ...
            resultCtx.strokeRect(topLeftX, topLeftY, width, height);
            
            // Draw label background
            const label = `${className} ${(confidence * 100).toFixed(1)}%`;
            resultCtx.font = "bold 14px Arial";
            const textWidth = resultCtx.measureText(label).width;
            resultCtx.fillStyle = "rgba(0, 255, 0, 0.8)";
            resultCtx.fillRect(topLeftX, topLeftY - 22, textWidth + 10, 22);
            
            // Draw label text
            resultCtx.fillStyle = "#000000";
            resultCtx.fillText(label, topLeftX + 5, topLeftY - 6);
          });
        };

        // Run AI every 30 frames for better detection responsiveness
        ...
        try {
           ...
           setDetectionStatus(`${newPredictions.length} person(s) detected`);

           // Draw new predictions immediately
